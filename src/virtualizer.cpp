...
std::string Virtualizer::emitVirtualizedScript(const Bytecode& encrypted,
    ...
    s << "if k==\"_G\" or k==\"_ENV\" then return RealG end\n";
    ...
    s << "if k==\"RaycastParams\" then return RaycastParams end\n";
    ...
    s << "if k==\"xpcall\" then return xpcall end\n";
    ...
    s << "if k==\"require\" then return require end\n";
    s << "if k==\"shared\" then return shared end\n";
    s << "if k==\"setclipboard\" then return setclipboard end\n";
    s << "if k==\"setmetatable\" then return setmetatable end\n";
    s << "if k==\"getmetatable\" then return getmetatable end\n";
    s << "if k==\"rawget\" then return rawget end\n";
    s << "if k==\"rawset\" then return rawset end\n";
    ...
    s << "elseif op==" << n(Op::GETUPVAL) << " then\n";
    s << "local uv=ups[B+1]\n";
    s << "if uv==nil then uv=ups[B] end\n";
    s << "if uv==nil then uv=reg[B+1] end\n";
    s << "reg[Ra]=uv\n";
    s << "elseif op==" << n(Op::SETUPVAL) << " then\n";
    s << "ups[B+1]=reg[Ra]\n";
    s << "if ups~=reg then reg[B+1]=reg[Ra] end\n";
    ...
    s << "capt[ui]={ty=ca,idx=cb}\n";
    s << "end\n";
    s << "end\n";
    s << "local parentReg,parentUps=reg,ups\n";
    s << "reg[Ra]=function(...)\n";
    s << "local linked=setmetatable({},{__index=function(_,i)\n";
    s << "local c=capt[i]\n";
    s << "if c then\n";
    s << "if c.ty==2 then return parentUps[c.idx+1] end\n";
    s << "return parentReg[c.idx+1]\n";
    s << "end\n";
    s << "local v=parentReg[i]\n";
    s << "if v~=nil then return v end\n";
    s << "v=parentReg[i+1]\n";
    s << "if v~=nil then return v end\n";
    s << "return parentUps[i] or parentUps[i+1]\n";
    s << "end,__newindex=function(_,i,v)\n";
    s << "local c=capt[i]\n";
    s << "if c then\n";
    s << "if c.ty==2 then parentUps[c.idx+1]=v else parentReg[c.idx+1]=v end\n";
    s << "else parentReg[i]=v end\n";
    s << "end})\n";
    s << "return run(cid,{...},linked)\n";
    s << "end\n";
