...
std::string Virtualizer::emitVirtualizedScript(const Bytecode& encrypted,
    ...
    s << "for ui=1,nup do\n";
    s << "pc+=1\n";
    s << "if pc>#code then break end\n";
    s << "local cinst=code[pc]\n";
    s << "local ca=bit32.band(bit32.rshift(cinst,8),255)\n";
    s << "local cb=bit32.band(bit32.rshift(cinst,16),255)\n";
    s << "if ca==2 then mapU[ui]={t=\"u\",i=cb+1} else mapU[ui]={t=\"r\",i=cb+1} end\n";
    s << "end\n";
    s << "if nup>0 and next(mapU)==nil then for ui=1,nup do mapU[ui]={t=\"r\",i=ui} end end\n";
    ...
    s << "end})\n";
    s << "reg[Ra]=function(...) return run(cid,{...},proxy) end\n";
    s << "elseif op==" << n(Op::CAPTURE) << " then\n";
    s << "-- consumed by CLOSURE\n";
    s << "end\n";
    s << "pc+=1\n";
    s << "end\n";
    s << "end\n";
