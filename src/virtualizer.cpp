...
std::string Virtualizer::emitVirtualizedScript(const Bytecode& encrypted,
    ...
    // Capture Roblox APIs at load time (never rely on _G alone)
    s << "local G_game,G_workspace,G_script=game,workspace,script\n";
    s << "local G_Enum,G_Instance,G_Color3,G_UDim2,G_UDim=Enum,Instance,Color3,UDim2,UDim\n";
    s << "local G_Vector2,G_Vector3,G_CFrame=Vector2,Vector3,CFrame\n";
    s << "local G_RaycastParams,G_ColorSequence,G_ColorSequenceKeypoint=RaycastParams,ColorSequence,ColorSequenceKeypoint\n";
    s << "local G_TweenInfo,G_task,G_tick,G_time,G_os=TweenInfo,task,tick,time,os\n";
    s << "local G_typeof,G_pairs,G_ipairs,G_next=typeof,pairs,ipairs,next\n";
    s << "local G_pcall,G_xpcall,G_print,G_warn,G_error=pcall,xpcall,print,warn,error\n";
    s << "local G_tostring,G_tonumber,G_type,G_select=tostring,tonumber,type,select\n";
    s << "local G_table,G_string,G_math,G_bit32=table,string,math,bit32\n";
    s << "local G_require,G_shared=require,shared\n";
    s << "local G_setmetatable,G_getmetatable,G_rawget,G_rawset,G_rawequal=setmetatable,getmetatable,rawget,rawset,rawequal\n";
    s << "local G_setclipboard=setclipboard\n";
    ...
    s << "local fixed={\n";
    s << "_G=RealG,_ENV=RealG,\n";
    s << "game=G_game,workspace=G_workspace,Workspace=G_workspace,script=G_script,\n";
    s << "Enum=G_Enum,Instance=G_Instance,Color3=G_Color3,UDim2=G_UDim2,UDim=G_UDim,\n";
    s << "Vector2=G_Vector2,Vector3=G_Vector3,CFrame=G_CFrame,\n";
    s << "RaycastParams=G_RaycastParams,ColorSequence=G_ColorSequence,ColorSequenceKeypoint=G_ColorSequenceKeypoint,\n";
    s << "TweenInfo=G_TweenInfo,task=G_task,tick=G_tick,time=G_time,os=G_os,\n";
    s << "typeof=G_typeof,pairs=G_pairs,ipairs=G_ipairs,next=G_next,\n";
    s << "pcall=G_pcall,xpcall=G_xpcall,print=G_print,warn=G_warn,error=G_error,\n";
    s << "tostring=G_tostring,tonumber=G_tonumber,type=G_type,select=G_select,\n";
    s << "unpack=G_table and G_table.unpack,table=G_table,string=G_string,math=G_math,bit32=G_bit32,\n";
    s << "require=G_require,shared=G_shared,setclipboard=G_setclipboard,\n";
    s << "setmetatable=G_setmetatable,getmetatable=G_getmetatable,rawget=G_rawget,rawset=G_rawset,rawequal=G_rawequal,\n";
    s << "}\n";

    s << "local E=setmetatable({},{__index=function(_,k)\n";
    s << "local f=fixed[k]\n";
    s << "if f~=nil then return f end\n";
    s << "local v=rawget(RealG,k)\n";
    ...
    s << "elseif op==" << n(Op::GETTABLE) << " then local t=reg[Rb] reg[Ra]=t and t[reg[Rc]]\n";
    s << "elseif op==" << n(Op::SETTABLE) << " then local t=reg[Rb] if t then t[reg[Rc]]=reg[Ra] end\n";
    s << "elseif op==" << n(Op::GETTABLEKS) << " then pc+=1 local t=reg[Rb] local key=kn(p,code[pc]) reg[Ra]=t and t[key]\n";
    s << "elseif op==" << n(Op::SETTABLEKS) << " then pc+=1 local t=reg[Rb] local key=kn(p,code[pc]) if t then t[key]=reg[Ra] end\n";
    s << "elseif op==" << n(Op::GETTABLEN) << " then local t=reg[Rb] reg[Ra]=t and t[C+1]\n";
    s << "elseif op==" << n(Op::SETTABLEN) << " then local t=reg[Rb] if t then t[C+1]=reg[Ra] end\n";
    ...
    s << "elseif op==" << n(Op::CALL) << " then local narg=if B==0 then (#reg-A) else (B-1) local fn=reg[Ra] local argv={} for i=1,math.max(narg,0) do argv[i]=reg[Ra+i] end if type(fn)~=\"function\" then error(\"bad call \"..tostring(fn)) end local ret={fn(table.unpack(argv,1,math.max(narg,0)))} if C~=1 then local limit=if C==0 then #ret else (C-1) for i=1,limit do reg[Ra+i-1]=ret[i] end end\n";
